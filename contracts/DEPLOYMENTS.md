...
## Testnet — Phase 5 shielded pool (XLM, configurable KYC policy) — **CURRENT**
...
| Explorer           | https://stellar.expert/explorer/testnet/contract/CD645P75NWNDIYZZ3363ABNK6WL435KLYOQYKUPLHLMEXIF7QMI5WGUA   |
| Deploy tx          | https://stellar.expert/explorer/testnet/tx/fc19f73da33c7c66ecbd60760f9437b50922ce0883ee3149cbd45b20074ecec9 |
| Initialize tx      | https://stellar.expert/explorer/testnet/tx/6940cf7df76098e4b134413d4284d6124479aa5964ac7ab802601468116f0d98 |
| Anchor rotation tx | https://stellar.expert/explorer/testnet/tx/29dbe4067008874707416b2a34481d520086d68b22deea410719122f976e009f |
| Post-deploy checks | `min_kyc_level` 1 ✓ · `is_paused` false ✓ · `queue_depth` 0 ✓ · `root` = empty-tree root ✓ · `admin` ✓      |
| Anchor key         | `27262bd204b7923a…` / `063d8d9ae872afd0…` — matches the live backend, verified against `/anchors/trusted`   |

**Why this pool replaces `CBLLKI…`.** The minimum KYC level moved from a compiled-in circuit constant
to a stored policy supplied as public input 15 (`Docs/progress.md` §0.3). That changes the circuit's
public input count from 15 to 16, so the verifying key changed and the old contract cannot verify
proofs from the new prover — or vice versa. A redeploy was unavoidable, not preferred.

The policy is readable by anyone (`min_kyc_level()`) and changeable by the admin alone
(`set_min_kyc_level`), which emits an event. **Raising it invalidates in-flight proofs** built against
the old value, exactly as an anchor rotation does: the wallet re-reads `/pool/status` and re-proves.

**The anchor had to be rotated again, for the same reason as last time and with a different cause.**
The deploy script warned when `ANCHOR_SEED` was unset but then called `anchor-pubkey` _without
passing it_, so exporting the seed changed nothing and the pool initialised with the prover's
built-in dev key (`17ba7d68…`). Caught by reading `AnchorPk` off the chain immediately after
deployment rather than trusting the script's output. Fixed in the script — the seed is now passed
through — so the next deployment cannot repeat it.

## Testnet — Phase 4 shielded pool (XLM, superseded by CD645P…)
...
| Verify cost             | ~49.0M CPU / ~427KB mem (native); within the 100M cap (in-circuit KYC adds to _proving_, not verify)      |
